...
1.  Leonardo Makoto - 7180679

2.  Leonardo Lima - 14334311
...
summary(mod_simples)$adj.r.squared
summary(mod_quad)$adj.r.squared 
summary(mod_cubico)$adj.r.squared
...
# valor ótimo de lambda para o Ridge:
...
# Valor ótimo de lambda para o Lasso
...
```{r preliminares2, , warning=FALSE, include=FALSE}
...
cat("A taxa de erro de classificação será a soma das classificações erradas sobre total:",
    mean(log.previsao != Weekly_teste$Direction))
...
knn.previsao <- knn(as.matrix(Weekly_train$Lag2), 
                    as.matrix(Weekly_teste$Lag2), 
                    Weekly_train$Direction, k = 1)
```
...
```{r preliminares3, , warning=FALSE, include=FALSE}
...
previsores_menor_erro <- lapply(knn_results, 
                                function(x) min(sapply(x, function(y) y$knn_error))) %>% 
  which.min()

k_menor_erro <- k_menores_erros[[previsores_menor_erro]]

best_knn_0 <- min(unlist(lapply(knn_results, 
                                function(x) min(sapply(x, function(y) y$knn_error)))))
