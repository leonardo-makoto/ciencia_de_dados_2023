...
# O número máximo de fatores que podem ser extraídos é igual ao número de variáveis usadas na análise. 
# Para a análise atual, a observação preliminar das correlações sugerem que serão extraído dois fatores.
# Fator 1 (Finanças e Custos) e Fator 2 (Marketing)
...
# A partir deste ponto adicionamos a variável Nota Atuárias (4 variáveis no total)
...
# De onde vêm o valor crítico do chi-quadrado de Bartlett?
-((100-1)-((2*4+5)/6))*log(det(rho))
...
#--> sim, os dados são adequados (a matriz de correlação é diferente de uma matriz)
